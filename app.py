...
# Flask 앱 생성. index.html은 templates 폴더 내에 있어야 합니다.
app = Flask(__name__, template_folder="templates")

# API 키: 환경변수에서 가져오거나, 테스트용 기본값 사용 (반드시 실제 API 키로 교체)
API_KEY = os.environ.get("GEMINI_API_KEY", "AIzaSyA1T8A8V9PvBo6BsUwIimkAZNc13h0cwJw")

def get_db_connection():
    """
    환경 변수 DATABASE_URL에 저장된 PostgreSQL 연결 문자열을 사용하여 데이터베이스 연결을 반환합니다.
    """
...
@app.route('/data')
def data():
    # PostgreSQL DB에서 데이터를 읽어 JSON 반환
    conn = get_db_connection()
